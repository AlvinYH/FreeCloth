...
from lib.model.pointnet import ResnetPointnet
...
class PointNetfeat(nn.Module):
    ...
    def __init__(
        self, in_size=3, num_points=16384, global_feat=True, use_SElayer=False, hidden_size=4096
    ):
        super(PointNetfeat, self).__init__()
        self.use_SElayer = use_SElayer
        self.conv1 = torch.nn.Conv1d(in_size, 64, 1)
        self.conv2 = torch.nn.Conv1d(64, 128, 1)
        self.conv3 = torch.nn.Conv1d(128, hidden_size, 1)
...
class SpareNetGenerator(nn.Module):
    ...
    def __init__(
        ...
        self.outfit_code_size = outfit_code_size
        ...
        self.global_feat_extractor = PointNetfeat(
            in_size=6, 
            global_feat=True, 
            use_SElayer=True, 
            hidden_size=256
        ...
        self.global_encoder = nn.Sequential(
            self.global_feat_extractor,
            nn.Linear(256, 256),
            nn.BatchNorm1d(256),
            nn.ReLU(),
        )

        # part-based local feature encoder
        if encode == 'Pointfeat':
            self.encoder = PointNetPPEncoder(dim=6, use_xyz=False)
        else:
            raise NotImplementedError
        
        # morphing-based decoder
        self.decoder = SpareNetDecode(
            num_points=self.num_points,
            n_primitives=self.n_primitives,
            bottleneck_size=self.bottleneck_size, #+self.outfit_code_size,
            use_AdaIn=self.use_AdaIn,
            use_SElayer=use_SElayer,
        )
        
        # we omit the refinement module
        '''
    ...
    def forward(self, body_pc, outfit_code):
        n_batch, n_part, n_point, n_dim = body_pc.shape
        
        # extrac global feature (not including canonical points here)
        full_pc = body_pc.view(n_batch, n_part*n_point, n_dim).transpose(1, 2).contiguous() 
        global_style = self.global_encoder(full_pc[:, 3:])
        # global_style = torch.cat([global_style, outfit_code], -1)
        
        # encode pose style for each part
        body_pc = body_pc.view(n_batch * n_part, n_point, n_dim)  # [bs*part, 3, in_points]
        style = self.encoder(body_pc)  # [bs*part, 256, n_point_per_part]
        feat_dim = style.shape[-2]
        
        # use max pooling to get the part style
        part_style = nn.MaxPool1d(style.shape[-1])(style).squeeze(-1)  # [bs*part, 256]
        part_style = part_style.view(n_batch, n_part, -1)  # [bs, n_part, 256]
        
        # extract the global style
        # style = style.view(n_batch, n_part, feat_dim, style.shape[-1]).permute(0, 2, 1, 3)  # [bs, 256, n_part, n_point_per_part]
        # style = style.reshape(n_batch, feat_dim, -1)  # [bs, 256, n_point*n_part]
        # global_style = self.global_encoder(style)  # [bs, 256, n_point*n_part]
        # global_style = nn.MaxPool1d(global_style.shape[-1])(global_style).squeeze(-1)  # [bs, 256]
        
        all_style = torch.cat([part_style, global_style.unsqueeze(1).repeat(1, n_part, 1)], -1)
        # style = torch.cat([style, outfit_code], -1)  # [batch_size, 1024]

        # decode
        outs = self.decoder(all_style)
        coarse_pred = outs.transpose(1, 2).contiguous()  # [batch_size, out_points, 6]
        
        coarse_pc, normals = torch.split(coarse_pred, 3, dim=-1)
        normals = F.normalize(normals, dim=-1)
        return coarse_pc, normals
    
        # refine first time
        # refine_pc, loss_mst = self.refine(outs, partial, coarse)
    ...
    def __init__(
        ...
        if encode == "Residualnet":
            self.feat_extractor = EdgeConvResFeat(
                use_SElayer=use_SElayer, k=8, output_size=hidden_size, hidden_size=4096
            )
        elif encode == 'ResnetPointnet':
            self.feat_extractor = ResnetPointnet(dim=6, out_dim=hidden_size, hidden_dim=256)
        else:
            self.feat_extractor = PointNetfeat(
                in_size=in_size, global_feat=True, use_SElayer=use_SElayer, hidden_size=hidden_size
            )
        self.linear = nn.Linear(hidden_size, bottleneck_size)
...
class EdgeConvResFeat(nn.Module):
    ...
    def __init__(
        ...
        self.conv1 = nn.Conv2d(6, self.hidden_size // 16, kernel_size=1, bias=False)
        self.conv2 = nn.Conv2d(
            self.hidden_size // 8, self.hidden_size // 16, kernel_size=1, bias=False
        )
        self.conv3 = nn.Conv2d(
            self.hidden_size // 8, self.hidden_size // 8, kernel_size=1, bias=False
        )
        self.conv4 = nn.Conv2d(
            self.hidden_size // 4, self.hidden_size // 4, kernel_size=1, bias=False
        )
        self.conv5 = nn.Conv1d(
            self.hidden_size // 2, self.output_size // 2, kernel_size=1, bias=False
        )
        ...
        if use_SElayer:
            self.se1 = SELayer(channel=self.hidden_size // 16)
            self.se2 = SELayer(channel=self.hidden_size // 16)
            self.se3 = SELayer(channel=self.hidden_size // 8)
            self.se4 = SELayer(channel=self.hidden_size // 4)

        self.bn1 = nn.BatchNorm2d(self.hidden_size // 16)
        self.bn2 = nn.BatchNorm2d(self.hidden_size // 16)
        self.bn3 = nn.BatchNorm2d(self.hidden_size // 8)
        self.bn4 = nn.BatchNorm2d(self.hidden_size // 4)
        self.bn5 = nn.BatchNorm1d(self.output_size // 2)

        self.resconv1 = nn.Conv1d(
            self.hidden_size // 16, self.hidden_size // 16, kernel_size=1, bias=False
        )
        self.resconv2 = nn.Conv1d(
            self.hidden_size // 16, self.hidden_size // 8, kernel_size=1, bias=False
        )
        self.resconv3 = nn.Conv1d(
            self.hidden_size // 8, self.hidden_size // 4, kernel_size=1, bias=False
        )
...
class SpareNetDecode(nn.Module):
    """
    inputs:
    - all_style(feature): b x n_part x feature_size
    ...
    def __init__(
        ...
        # convert the grid to torch tensor and register as buffer
        # modified implementation
        grid = grid_generation(self.num_points, self.n_primitives)
        self.grid = torch.tensor(grid).float().cuda()
    ...
    def forward(self, all_style):
        n_part = all_style.shape[1]
        assert self.n_primitives % n_part == 0
        
        # all_style: [bs, n_part, 512+64]
        # assign different style to each primitive
        outs = []
        if self.use_AdaIn == "share":
            # generate adain parameters in a flattened batch
            style_flatten = all_style.view(-1, all_style.shape[-1])
            all_adain_params = self.mlp(style_flatten)
            all_adain_params = all_adain_params.view(-1, n_part, all_adain_params.shape[-1])
                    
            for i in range(self.n_primitives):
                regular_grid = self.grid[i]
                regular_grid = regular_grid.transpose(0, 1).contiguous().unsqueeze(0)
                regular_grid = regular_grid.expand(
                    all_style.size(0), regular_grid.size(1), regular_grid.size(2)
                ).contiguous()
                regular_grid = ((regular_grid - 0.5) * 2).contiguous()

                part_idx = i // (self.n_primitives // n_part)
                style = all_style[:, part_idx]
                adain_params = all_adain_params[:, part_idx]
                
                # generate adain parameters for each part
        ...
        #         regular_grid = regular_grid.transpose(0, 1).contiguous().unsqueeze(0)
        #         regular_grid = regular_grid.expand(
        #             style.size(0), regular_grid.size(1), regular_grid.size(2)
        #         ).contiguous()
        #         regular_grid = ((regular_grid - 0.5) * 2).contiguous()
...
class GridDecoder(nn.Module):
    ...
    def __init__(
        ...
        if not self.use_sine:
            ...
            self.linear5 = nn.Conv1d(self.bottleneck_size // 4, 6, 1)
